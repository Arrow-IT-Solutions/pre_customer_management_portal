...
export class RenewComponent implements OnInit {
  ...
  companyName:string;
   ...
   if (!oldEndDate || !periodNumber || !periodTypeKey) {
    ...
    console.log('Subscription',this.subscriptionService.SelectedData)

   if(this.subscriptionService.SelectedData != null){
    //this.companyName=this.subscriptionService.SelectedData?.companyService?.company?.companyTranslation?[this.layoutService.config.lang]?.name
   }

    if(this.renewService.SelectedData != null){
      console.log("response",this.renewService.SelectedData)
     // this.companyName=this.renewService.SelectedData?.companyService?.company?.companyTranslation!['en'].name
      await this.FillData();
